...
app.get("/", (req, res) => {
  res.send(`
    <h1>BSIT 1E Deal or No Deal</h1>

    <ul>
      <li><a href="/tv">Open TV Display</a></li>
      <li><a href="/controller">Open Controller</a></li>
      <li><a href="/group">Open Group Device</a></li>
      <li><a href="/teacher">Open Teacher Device (Ginoong Ricky)</a></li>
    </ul>
  `);
});
...
function broadcastGameState() {
  ...
  if (gameState.activeBriefcase.caseNumber) {
    ...
    publicState.currentQuestion = questions[caseNum] ?? null;
  } else {
...
function handleCorrectAnswer() {
  ...
  // Determine value based on OPEN ORDER (rigged sequence).
  const openIndex = gameState.briefcases.openOrder.length;
  const value = gameState.briefcases.fixedValueSequence[openIndex];
...
function revealFinalLuckyBriefcase() {
  ...
  // Assign the final value (always last in sequence = ₱30).
  const openIndex = gameState.briefcases.openOrder.length;
  const value = gameState.briefcases.fixedValueSequence[openIndex];

  gameState.briefcases.cases[luckyCase].isOpened = true;
  gameState.briefcases.cases[luckyCase].assignedValue = value;
  gameState.briefcases.cases[luckyCase].openedByGroup = luckyStarGroup;
  gameState.briefcases.openOrder.push(luckyCase);
  gameState.briefcases.claimedValues.push(value);

  // Add value to Lucky Star's group score.
  if (luckyStarGroup && gameState.groups[luckyStarGroup]) {
    gameState.groups[luckyStarGroup].score += value;
  }

  // Set the active briefcase to show the reveal.
  ...
  console.log(
    `🎉 LUCKY BRIEFCASE REVEALED! Briefcase ${luckyCase} = ₱${value} → Group ${luckyStarGroup}`
  );
  ...
  return {
    success: true,
    message:
      `Lucky Briefcase ${luckyCase} revealed: ₱${value}! ` +
      `Added to Group ${luckyStarGroup}'s score.`
  };
}
...
io.on("connection", (socket) => {
  ...
  socket.on("register-device", (role) => {
    const allowedRoles = ["tv", "controller", "group"];
  ...
  */
  ...
  socket.on("group-login-attempt", (data, callback) => {
    ...
    if (
      ...
      return callback({
        success: false,
        message: `Group ${groupNumber} is already logged in on another device.`
      });
    }
