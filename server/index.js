...
const gameState = {
  ...
  briefcases: {
    ...
    autoCalledRound: 0                         // Highest round already auto-called (auto banker call)
  },
...
function broadcastGameState() {
  ...
  io.to("group").emit("buzzer-state", {
    ...gameState.buzzer,
    pauseActive: gameState.pauseOverlay.isActive
  });
}
...
// Automatically trigger the banker call once a new round threshold is reached.
// Thresholds (cases opened): 3 → round 1, 5 → round 2, 6 → round 3,
// 7 → round 4, 8 → round 5 (i.e. after 3, then +2, +1, +1, +1 cases).
function maybeAutoBankerCall() {
  const casesOpened = gameState.briefcases.openOrder.length;
  const round = bankerFormula.determineCurrentRound(casesOpened);

  // Not enough cases for an offer yet, or this round was already called.
  if (round === 0) return null;
  if (round <= gameState.banker.autoCalledRound) return null;

  const result = triggerBankerCall();

  // Only advance if the call actually succeeded (e.g. Lucky Star group is set).
  if (result && result.success) {
    gameState.banker.autoCalledRound = round;
  }
  return result;
}
...
function revealBankerOffer(roundNumber) {
  ...
  if (gameState.banker.computedOffer === null) {
    return {
      success: false,
      message: "Wala pang offer. Maghintay sa awtomatikong tawag ng banker."
    };
  }
...
function handleNoDeal() {
  ...
  // Reset lastDecision right away so the NEXT no-deal creates a fresh
  // "null → no_deal" transition on the TV (otherwise the TV's sound trigger
  // only sees the flag already set and stays silent on later rounds).
  gameState.banker.lastDecision = null;
...
function goToHomepageFromBriefcase() {
  ...
  // Auto banker call once a new round threshold is reached.
  maybeAutoBankerCall();
...
// Emergency escape hatch: clears the pause overlay AND force-logs-out the
// teacher, so the controller can recover without restarting the server
// (e.g. sir left while paused, leaving the pause stuck ON).
function forceClearPause() {
  gameState.pauseOverlay.isActive = false;
  gameState.pauseOverlay.activatedAt = null;

  if (gameState.teacher.isLoggedIn) {
    const tSid = gameState.teacher.socketId;
    gameState.teacher.isLoggedIn = false;
    gameState.teacher.socketId = null;
    if (tSid) {
      const tSocket = io.sockets.sockets.get(tSid);
      if (tSocket) {
        tSocket.data.isTeacher = false;
        tSocket.emit("force-logout");
      }
    }
  }
  ...
  return {
    success: true,
    message: "Pause cleared and teacher logged out."
  };
}
  ...
  socket.on("controller-action", (actionData, callback) => {
    ...
    switch (action) {
      ...
      case "BUZZER_TEST_END":
        ...
        break;
      case "FORCE_CLEAR_PAUSE":
        result = forceClearPause();
        break;
      case "SET_GAME_MODE":
